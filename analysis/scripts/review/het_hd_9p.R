...
df$Subject <- df$Patient.y
clinical_data <- merge(clinical_data, df)
clinical_data <- clinical_data %>%
    group_by(Subject) %>%
    dplyr::summarise(
        status_9p = case_when(
            any(status_9p == "HD") ~ "HD",
            any(status_9p == "LOH") & !any(status_9p == "HD") ~ "LOH",
            TRUE ~ "WT"
        ),
        # now just regrab clinical info
        pfs_time = unique(pfs_time),
        pfs = unique(pfs),
        stage_simple = unique(stage_simple)
    )

table(clinical_data$stage_simple, clinical_data$status_9p)
coxph(Surv(pfs_time, pfs) ~ status_9p, data = clinical_data)

km_fit <- survfit(Surv(pfs_time, pfs) ~ status_9p,
    data = clinical_data
)
...
plt <- brewer.pal(3, "Set1")
km_plot <- plot_km(clinical_data, km_fit, plt = plt)
save_baseplot(km_plot, file.path(FIG_DIR, "surv_hd_loh_wt_9p"), w = 100, h = 100)
km_plot

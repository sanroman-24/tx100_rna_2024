...
p <- ssgsea_difs_df %>% 
  ...
  group_by(Functional_group) %>% summarise(p_value = hmp.stat(p_value), t_value = mean(t_value)) %>% 
...
save_ggplot(p, file.path(FIG_DIR, "ssgsea_paired_primary_met_lolli"), h = 45, w = 50)